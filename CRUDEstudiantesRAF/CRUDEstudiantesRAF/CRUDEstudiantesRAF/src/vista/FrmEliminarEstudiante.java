...
public class FrmEliminarEstudiante extends javax.swing.JFrame {
    ...
    private void txtNumControlKeyPressed(java.awt.event.KeyEvent evt) {//GEN-FIRST:event_txtNumControlKeyPressed
        // TODO add your handling code here:
         if(evt.getKeyCode() == KeyEvent.VK_ENTER) {
        // Verificar que el campo no esté vacío antes de mover el foco
        if (!txtNumControl.getText().trim().isEmpty()) {
            this.buscar();
            this.btnEliminar.requestFocus(); // Mover el foco al botón de eliminar
        }
    }
...
private void buscar() {
    ...
    if (registro != null && registro.length >= 6) {
        ...
        // Validación del semestre antes de asignar
        if (!registro[3].trim().isEmpty()) {
            try {
        ...
        Map<String, JRadioButton> grupos = Map.of(
            "A", rdbtnGrupoA,
            "B", rdbtnGrupoB,
            "C", rdbtnGrupoC
        );

        JRadioButton grupoSeleccionado = grupos.getOrDefault(registro[4], null);
        if (grupoSeleccionado != null) {
            grupoSeleccionado.setSelected(true);
        }

        this.cboCarrera.setSelectedItem(registro[5]);

        // Solo habilitar el botón de eliminar
        this.btnEliminar.setEnabled(true);
        
    } else {
        JOptionPane.showMessageDialog(this, "Número de control no encontrado o registro incompleto.");
        limpiarCampos();
...
private void eliminar(){
    int respuesta = JOptionPane.showConfirmDialog(this, "¿Estás seguro de eliminar el registro?");
